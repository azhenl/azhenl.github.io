...
window.onload = function() {
    ...
    var j_box = $("j_box");  // 获取最大盒子
    var j_banner = $("j_banner");  // 滚动图片的父亲
    var imgs = j_banner.children;  // 获得所有的图片组 需要滚动的部分
    var j_square = $("j_square");  // 获得 控制span 的 父盒子
    // 操作元素
    // 生成小span
    for(var i=0;i<imgs.length; i++) {

        var li = document.createElement("li");// 创建 span
        li.className = "slider-square"; // 添加类名
        li.innerHTML = imgs.length-i;  //  6 - 0     6 - 1   // 实现 倒序 的方式插入
        j_square.insertBefore(li,j_square.children[1]);  // 再 父亲 倒数第二个盒子的前面插入
    }
    // 下面的第一个小span  是默认的蓝色
    var lis = j_square.children;   // 得到所有的 span
    lis[1].setAttribute("class","slider-square current");  // 两个类名

    var scrollWidth = j_box.clientWidth; // 得到大盒子的宽度 也就是  后面动画走的距离  310
    ...
    for(var k in lis){   //   k  是索引号  spans[k]    spans[0]  第一个span
        lis[k].onclick = function() {
            ...
            if(this.className == "slider-arr"){ // 判断当前点击的这个按钮是不是 prev
//                 alert("您点击了左侧按钮");
...
//              alert(980);
            ...
            else if(this.className == "slider-arr2") {  // 右侧按钮开始
                autoplay();
            }
            else {
                // alert("您点击了下面的span");
    ...
    function setSquare() {
       //  清除所有的span current   留下 满足需要的拿一个
        for(var i=1;i<lis.length-1;i++){   //  8个span   我们要 1-6  不要 7  索引号
            lis[i].className = "slider-square";
        }
        lis[iNow+1].className = "slider-square current";  // 记住 + 1
    }
    // 定时器开始  其实， 定时器就是  右侧按钮
    var timer = null;
